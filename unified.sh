encoding_string=$(file $1)

file_to_translate=$1
if ! [[ $encoding_string =~ "UTF-8" ]]; then
    iconv -f gb2312 -t UTF-8 -o robot-unicode.STEP $1
    file_to_translate=robot-unicode.STEP
fi

grep -P '[\p{Han}]' $file_to_translate >> chinese-lines.txt
